...
mat_bin <- read.delim("/mnt/storage12/emma/PR_combine/MT.mat.bin")
mat_bin <- read.delim("/mnt/storage12/emma/PR_combine/VGSC.mat.bin")
...
plot <- ggplot() + 
    ...
    geom_point(data = df, aes(x = PC1, y = PC2, colour = Country), size = 5) +
    xlab(paste0("PC1 ", PC1_lab, "%")) +
    ylab(paste0("PC2 ", PC2_lab, "%")) +
    #scale_colour_manual(values = col_ramp, name = "Country") +
    scale_colour_manual(values = c("#40476D", "#678D58", "#EE964B", "#F95738",  "#A6C48A",  "#51A3A3", "#7D387D", "#bc80bd"),
                        breaks = c("Puerto Rico", "Thailand", "Burkina Faso", "Kenya", "Uganda", "USA", "Mexico", "Brazil"),
                        limits = c("Puerto Rico", "Thailand", "Burkina Faso", "Kenya", "Uganda", "USA", "Mexico", "Brazil")) +
    theme_classic() +
    #ggtitle("Voltage Gated Sodium Channel") +
...
plot
tiff("/mnt/storage12/emma/PR_combine/PCA_plots/PCA_all_samples_PC1_PC2_lmiss_newcolours.tiff",height=1200,width=1200)
dev.off()
...
####### PC1 vs PC3 #######
plot <- ggplot() + 
    #geom_point(data = df, aes(x = PC1, y = PC3, colour = !!sym(colour_by)), size = 4) +
    geom_point(data = df, aes(x = PC1, y = PC3, colour = Country), size = 4) +
    xlab(paste0("PC1 ", PC1_lab, "%")) +
...
plot
tiff("/mnt/storage12/emma/PR_combine/PCA_plots/PCA_all_samples_PC1_PC3_lmiss_newcolours.tiff",height=1200,width=1200)
dev.off()
...
plot <- ggplot() + 
    ...
    geom_point(data = df, aes(x = PC2, y = PC3, colour = Country), size = 4) +
...
plot
tiff("/mnt/storage12/emma/PR_combine/PCA_plots/PCA_all_samples_PC2_PC3_lmiss_newcolours.tiff",height=1200,width=1200)
dev.off()
...
write.tree(tree_dat, file="all_samples_lmiss_VGSC.tree")
